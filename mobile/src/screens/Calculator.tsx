...
export const Calculator = () => {
  ...
  const fetchBillHistory = async () => {
    const bills = await getCarbonHistory(5);
  ...
  if (savedStats && showResults && !travelDistance) {
    ...
    const dailyTravelEmission =
      (parseFloat(travelDistance) || 0) * selectedVehicle.emissionFactor;
    ...
    if (travelDistance && electricity && age) {
      ...
      try {
        ...
        const annualEmission =
          (dailyTravelEmission + dailyElectricityEmission) * 365;
    ...
    >
      ...
      {showResults && (
        <>
          ...
          >
            <View
              style={[styles.resultHeader, { justifyContent: "space-between" }]}
            >
              <View
            ...
            <Text style={styles.emissionValue}>
              {formatNumber(lifetimeEmission / 1000)} Tonnes
            </Text>
            <Text style={styles.emissionLabel}>
              Total expected CO₂ emission for remaining life
            </Text>
          ...
          {TREE_SPECIES.map((tree) => {
            const specificTreesNeeded = Math.ceil(
              lifetimeEmission / (tree.absorption * 40),
            );
              ...
              <View key={tree.name} style={styles.speciesCard}>
                ...
                <View style={styles.speciesInfo}>
                  ...
                  <Text style={styles.speciesDetail}>
                    Absorbs ~{tree.absorption} kg CO₂/year
                  </Text>
                </View>
          ...
          <View style={styles.infoBox}>
            ...
            <Text style={styles.infoText}>
              Calculations are based on average emission factors and a tree
      ...
      <Card style={styles.inputCard} elevation={0}>
        <View style={styles.cardHeader}>
          ...
          <Text style={styles.cardTitle}>Scan Utility Bill (OCR)</Text>
        </View>
        <Text
          style={{
            fontSize: 13,
        ...
        {billUploading ? (
          <View style={{ alignItems: "center", paddingVertical: 30 }}>
            ...
            >
              Processing bill with OCR...
            </Text>
          </View>
        ) : billResult ? (
          <View style={styles.billResultCard}>
            <Text
              style={{
                fontSize: 14,
                fontWeight: "700",
                color: billResult.success ? "#00695C" : "#E65100",
                marginBottom: 8,
              }}
            >
              {billResult.success
                ? "✅ Bill Processed"
                : "⚠️ " + billResult.message}
            </Text>
            {billResult.success && (
              <>
                <Text
                ...
                >
                  {billResult.carbonKg?.toFixed(1)} kg CO₂
                </Text>
                <Text
                  style={{
                    fontSize: 12,
          ...
          <View style={{ flexDirection: "row", gap: 12 }}>
            <TouchableOpacity
              ...
              onPress={async () => {
                ...
                if (!result.canceled && result.assets?.[0]) {
                  ...
                  if (res.success) fetchBillHistory();
                }
              }}
            >
            ...
            <TouchableOpacity
              style={[styles.scanBtn, { backgroundColor: "#0277BD" }]}
              onPress={async () => {
                const { status } =
      ...
      {billHistory.length > 0 && (
        <>
          ...
          {billHistory.map((bill: any, i: number) => (
            <View key={bill.id || i} style={styles.speciesCard}>
              <View style={styles.speciesIconBg}>
              ...
              <View style={styles.plantCount}>
                <Text style={styles.countValue}>
                  {bill.carbonKg?.toFixed(1)}
                </Text>
                <Text style={styles.countLabel}>kg CO₂</Text>
              </View>
            </View>
          ))}
        </>
