...
export const uploadBill = async (
  ...
  try {
    ...
    try {
      ...
      throw new AppError(
        "OCR processing failed. Please enter units manually.",
        422,
      );
    }
    ...
    if (!totalUnits || totalUnits <= 0) {
      return res.status(200).json({
        success: false,
        message:
          "Could not extract consumption units from the bill. Please enter manually.",
 ...
 *     security: [{ bearerAuth: [] }]
 */
export const manualBillEntry = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
