 ...
 */
export const extractUnitsFromText = (
  rawText: string,
): { totalUnits: number | null; patterns: string[] } => {
  const patterns: string[] = [];
  let totalUnits: number | null = null;

  // Pattern 1: "Total Units: 245" or "Units Consumed: 245" or "consumption 44kwh"
  const unitsPattern =
    /(?:total\s*units|units?\s*consumed|consumption|used)\s*[:\-]?\s*([\d,]+(?:\.\d+)?)/gi;
  ...
  // Pattern 2: "245 kWh" or "245 KWH" or "44kwh"
  if (!totalUnits) {
    const kwhPattern = /([\d,]+(?:\.\d+)?)\s*(?:kwh|units|kw)/gi;
  ...
  if (!totalUnits) {
    const amountPattern =
      /(?:amount|energy)\s*consumed\s*[:\-]?\s*([\d,]+\.?\d*)/gi;
  ...
  // Pattern 5: Fallback - look for large numbers near "units", "kwh", "consumption"
  if (!totalUnits) {
    const loosePattern =
      /(?:units|kwh|consumption|total|billed)\s*(?:is|:|-)?\s*([\d,]+(?:\.\d+)?)/gi;
    match = loosePattern.exec(rawText);
    if (match) {
      totalUnits = parseFloat(match[1].replace(",", ""));
      patterns.push(`loose_fallback_pattern: ${match[0]}`);
    }
  }

  // Pattern 6: Desperate Fallback - just find "kWh" and take the number before it
  if (!totalUnits) {
    const desperatePattern = /([\d,]+(?:\.\d+)?)\s*(?:kwh|units)/gi;
    match = desperatePattern.exec(rawText);
    if (match) {
      totalUnits = parseFloat(match[1].replace(",", ""));
      patterns.push(`desperate_pattern: ${match[0]}`);
    }
  }
 ...
 */
export const getUserCarbonBills = async (
  userId: string,
  limit: number = 20,
) => {
