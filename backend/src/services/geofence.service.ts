...
export const verifyAttendance = async (
    ...
    if (plantationEvent.status === 'completed') {
        return { verified: false, message: 'This event has already been completed' };
    }
    ...
    if (!isWithinBoundary) {
        return {
            verified: false,
            message: 'You are not within the event boundary. Please move closer to the planting site.',
        };
    }
...
export const createPlantationEvent = async (data: {
    ...
    const coordsStr = data.boundary.map(([lng, lat]) => `${lng} ${lat}`).join(', ');
    // Ensure the polygon is closed
    const firstCoord = data.boundary[0];
    const lastCoord = data.boundary[data.boundary.length - 1];
    const closedCoordsStr = (firstCoord[0] === lastCoord[0] && firstCoord[1] === lastCoord[1])
        ? coordsStr
        : `${coordsStr}, ${firstCoord[0]} ${firstCoord[1]}`;

    // Calculate centroid for easy point-based lookups
    const centroidLng = data.boundary.reduce((sum, [lng]) => sum + lng, 0) / data.boundary.length;
    const centroidLat = data.boundary.reduce((sum, [, lat]) => sum + lat, 0) / data.boundary.length;
    ...
    const result = await prisma.$executeRaw`
        INSERT INTO "PlantationEvent" (
            ...
            ${new Date(data.date)},
            ${data.locationName},
            ST_SetSRID(ST_GeomFromText('POLYGON((${closedCoordsStr}))', 0), 4326),
    ...
    return { created: result > 0, centroid: { lng: centroidLng, lat: centroidLat } };
};
